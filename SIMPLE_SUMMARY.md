...
只修改了 `src/vllm_router/routers/routing_logic.py` 文件，為兩個 kvaware router 添加了基本的性能監控功能：

### 修改的 Router 類：
1. **TtftRouter** - TTFT kvaware router
2. **KvawareRouter** - 標準 kvaware router
...
## 功能
- 測量每個主要步驟的執行時間
- 當 count > 100 時，自動保存到 CSV 文件
- 文件保存位置：`/home/w00917303/test_result.csv`

## 監控的步驟

### TtftRouter 監控步驟：
1. tokenize_time - Tokenize 步驟時間
2. lookup_time - KV cache lookup 時間  
3. find_best_matched_time - 尋找最佳匹配時間
4. find_best_ttft_time - 尋找最佳 TTFT 時間
5. fallback_time - 回退路由時間
6. total_time - 總執行時間

### KvawareRouter 監控步驟：
1. tokenize_time - Tokenize 步驟時間
2. lookup_time - KV cache lookup 時間
3. hash_routing_time - Hash 路由時間
4. instance_mapping_time - Instance 映射時間
5. total_time - 總執行時間
